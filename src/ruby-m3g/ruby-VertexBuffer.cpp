...
VALUE ruby_VertexBuffer_get_tex_coords (VALUE self)
{
    ...
    Data_Get_Struct (self, VertexBuffer, p);
    val_accessor = Data_Make_Struct (rb_cVertexBuffer_TextureAccessor, TextureAccessor, 0, -1, accessor);
...
VALUE ruby_VertexBuffer_set_positions (VALUE self, VALUE val_args)
{
  ...
  scale = RFLOAT_VALUE(val_scale);
  bias[0] = RFLOAT_VALUE(rb_ary_entry(val_bias, 0));
  bias[1] = RFLOAT_VALUE(rb_ary_entry(val_bias, 1));
  bias[2] = RFLOAT_VALUE(rb_ary_entry(val_bias, 2));
